...
if __name__ == "__main__":
    ...
    V_star, pi_star = value_iteration(
        env_with_model, np.zeros(env_with_model.spec.nS), 1e-4
    )

    assert np.allclose(V_star, np.array([1.0, 0.0]), 1e-5, 1e-2), V_star
    ...
    assert np.allclose(V, np.array([1.0, 0.0]), 1e-5, 1e-2), V
    assert np.allclose(Q, np.array([[1.0, 0.0], [0.0, 0.0]]), 1e-5, 1e-2), Q

    V, Q = value_prediction(
        env_with_model, behavior_policy, np.zeros(env.spec.nS), 1e-4
    )
    assert np.allclose(V, np.array([0.1, 0.0]), 1e-5, 1e-2), V
    assert np.allclose(Q, np.array([[0.19, 0.0], [0.0, 0.0]]), 1e-5, 1e-2), Q
    ...
    Q_est_ois = mc_ois(
        ...
        behavior_policy,
        behavior_policy,
        np.zeros((env.spec.nS, env.spec.nA)),
    )
    Q_est_wis = mc_wis(
        env.spec,
        trajs,
    ...
    assert np.allclose(
        Q_est_ois, np.array([[0.19, 0.0], [0.0, 0.0]]), 1e-5, 1e-1
    ), "due to stochasticity, this test might fail"
    assert np.allclose(
        Q_est_wis, np.array([[0.19, 0.0], [0.0, 0.0]]), 1e-5, 1e-1
    ), "due to stochasticity, this test might fail"
    assert np.allclose(
        Q_est_ois, Q_est_wis
    ), "Both implementation should be equal in on policy case"
    assert np.allclose(
        V_est_td, np.array([0.1, 0.0]), 1e-5, 1e-1
    ), "due to stochasticity, this test might fail"
